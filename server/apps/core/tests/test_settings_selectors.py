"""Tests for apps.core.selectors_settings and presentation_settings.

Verifies the read-side config value retrieval and the presentation
layer's formatting and context shaping.
"""
...
def _build_context(user):
    """Helper: compose selector + presentation to get the full settings context."""
    company_id = getattr(getattr(user, 'company', None), 'id', None)
    config_values = get_all_config_values(company_id)
    return build_settings_context(user=user, config_values=config_values)
